...
<!--				<hr/>-->
    ...
    <!-- ##################################### TEMPLATES ZUR UMWANDLUNG DER BUCHSTABEN ETC. ##################################### -->
    
    <xsl:template match="//text()">
        <!--<xsl:variable name="vTextREPLACED">
<!-\-			<xsl:choose>
				<xsl:when test="contains(.,'ae') or contains(.,'ę') or contains(.,'v') or contains(.,'ci') ">
			...
			<xsl:variable name="vTextREPLACEDae">
			...
			<xsl:variable name="vTextREPLACEDecaudata">
				<xsl:call-template name="string-replace">
					<xsl:with-param name="string" select="$vTextREPLACEDae"/>
					<xsl:with-param name="replace" select="'ę'"/>
					<xsl:with-param name="with" select="'e'"/>
				</xsl:call-template>
			</xsl:variable>
			
			<xsl:variable name="vTextREPLACEDv">
				<xsl:call-template name="string-replace">
					<xsl:with-param name="string" select="$vTextREPLACEDecaudata"/>
					<xsl:with-param name="replace" select="'v'"/>
					<xsl:with-param name="with" select="'u'"/>
				</xsl:call-template>
			</xsl:variable>
			
			<xsl:variable name="vTextREPLACEDci">
				<xsl:call-template name="string-replace">
					<xsl:with-param name="string" select="$vTextREPLACEDv"/>
					<xsl:with-param name="replace" select="'ci'"/>
					<xsl:with-param name="with" select="'ti'"/>
				</xsl:call-template>
			</xsl:variable>
			
			<xsl:value-of select="$vTextREPLACEDci"/>
	...
	<xsl:template match="//tei:subst">
		...
		<xsl:choose>
			<xsl:when test="not(tei:add/@hand)">
			    ...
			    <xsl:call-template name="tTextNormalisierung">
			        <xsl:with-param name="pText" select="tei:add"/>
			    </xsl:call-template>
			</xsl:when>
			...
			<xsl:when test="string-length(tei:add/@hand)!=string-length(translate(tei:add/@hand,$vHandXYZ,''))">
			    ...
			    <xsl:call-template name="tTextNormalisierung">
			        <xsl:with-param name="pText" select="tei:del"/>
			    </xsl:call-template>
			</xsl:when>			
		</xsl:choose>
		...
		<xsl:choose>
			<xsl:when test="$vNoteFolgt='true'">
				...
				<xsl:if test="not(string-length(@hand)!=string-length(translate(@hand,$vHandXYZ,'')))">
				    ...
				    <xsl:call-template name="tTextNormalisierung">
				        <xsl:with-param name="pText" select="node()"/>
				    </xsl:call-template>
				</xsl:if>
...
<!--	<xsl:template match="//tei:seg[@xml:id]">
	...
	<xsl:template match="//tei:span[@xml:id]">
		...
		<!--<xsl:value-of select="node()[1]"/>-->
	    <xsl:call-template name="tTextNormalisierung">
	        <xsl:with-param name="pText" select="node()[1]"/>
	    </xsl:call-template>
	    
		<xsl:apply-templates select="tei:add"/>
		<!--<xsl:value-of select="$before"/>-->
	    <xsl:call-template name="tTextNormalisierung">
	        <xsl:with-param name="pText" select="$before"/>
	    </xsl:call-template>
